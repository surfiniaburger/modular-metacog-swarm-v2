...
3. **Metacognitive Flatness (Gemini 2.5 Flash & Claude Sonnet 4.6):** On our expanded 41-item diversity set, both models exhibited high accuracy (~81-86%) but extremely low metacognitive efficiency. Gemini 2.5 Flash registered an **`m_ratio = 0.050`**, and Claude Sonnet 4.6 registered an **`m_ratio = 0.054`**. This exposes a "Capability Chasm" where models can perform complex tasks but lack the internal resolution to know when they are failing. This also highlights the **Opus-Sonnet Divergence**, where the smaller, faster model (Sonnet) lacks the metacognitive monitoring of its larger sibling (Opus).
