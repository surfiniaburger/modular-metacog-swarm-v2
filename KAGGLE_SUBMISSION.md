...
4. **Calibrated Monitoring (DeepSeek V3.2 & Gemini 3 Flash Preview):** A significant breakthrough was observed in **Gemini 3 Flash Preview**, which achieved an **`m_ratio = 0.536`** (`96.0%` accuracy). Unlike its predecessors (Gemini 2.5 Flash, 3.1 Flash-Lite) which exhibited metacognitive flatness, Gemini 3 Flash demonstrates a "Generational Leap"—successfully integrating high-fidelity self-monitoring into a speed-optimized tier. It now matches **DeepSeek V3.2** (`m_ratio = 0.546`), both models actively modulating their internal confidence based on trial difficulty and evidence ambiguity.
